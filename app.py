        ...
        with tab1:
            if st.button("Summary Banao"):
                with st.spinner("AI padh raha hai..."):
                    st.write(model.generate_content(f"Summarize these notes in detail:\n\n{text[:10000]}").text)

        with tab2:
            if st.button("Flashcards Banao"):
                with st.spinner("Flashcards ban rahe hain..."):
                    st.write(model.generate_content(f"Create 5 important flashcards with Question and Answer format from these notes:\n\n{text[:10000]}").text)

        with tab3:
            if st.button("Practice Quiz Banao"):
                with st.spinner("Quiz generate ho raha hai..."):
                    st.write(model.generate_content(f"Create a 5-question multiple choice quiz with answers at the end based on these notes:\n\n{text[:10000]}").text)
else:
